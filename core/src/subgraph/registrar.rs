...
fn create_subgraph_version(
    ...
    let start_block: Option<Box<dyn Future<Item = EthereumBlockPointer, Error = EthereumAdapterError> + Send>> = manifest
        .data_sources
        .clone()
        .into_iter()
    ...
    if !deployment_exists {
        ...
        ops.extend(
            SubgraphDeploymentEntity::new(&manifest, false, false, earliest_block, chain_head_block)
